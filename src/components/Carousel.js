...
const Carousel = () => {
  ...
  return listofTopRestro.length === 0 ? (
    <Shimmer />
  ) : (
              ...
              <div className="bg-white rounded-lg overflow-hidden w-full h-full mx-auto transition-transform duration-300 ease-in-out transform hover:scale-95 cursor-pointer">
                ...
                <div className="p-1">
                  ...
                  <div className="flex justify-between text-md font-bold text-gray-600 mb-1 items-center">
                    ...
                    <span>
                      {card.info.sla ? card.info.sla.slaString : "N/A"}
                    </span>
                  </div>
                  <p className="text-gray-500 text-sm">
                    {card.info.cuisines && Array.isArray(card.info.cuisines)
                      ? card.info.cuisines.join(", ")
                      : "N/A"}
                  </p>
                  <p className="text-gray-500 text-sm">{card.info.areaName}</p>
                </div>
              </div>
            </Link>
          </div>
